...
//  - a 'devenv' file, supported by  https://github.com/motdotla/dotenv
//
//  If a devenv file exists, the contents of it will read and then it will be
...
const mergedEnvVars = fs.existsSync(envFile)
  // We have a devenv file, which we need to merge with the standard vars.
  ? Object.assign(
    {},
    // Merge the standard "process.env" environment variables object.
    process.env,
    // With the items from our "devenv" file
    dotenv.parse(fs.readFileSync(envFile, 'utf8'))
  )
  // No devenv file, so we will just use standard vars.
  : process.env;
...
if (!finalEnvVars.BUNDLE_OUTPUT_PATH || !finalEnvVars.BUNDLE_ASSETS_FILENAME) {
  throw new Error('Some of the critical environment variables are missing, please ensure that you have provided them via the command line or via a devenv file.');
}
