...
import {apiGitHub as api} from '../../services';
...
const firstPageStarredUrl = login => `https://api.github.com/users/${login}/starred`;
const firstPageStargazersUrl = fullName => `https://api.github.com/repos/${fullName}/stargazers`;
...
//  if (!userObj || requiredFields.some(key => !userObj.hasOwnProperty(key))) {
  if (!userObj || requiredFields.some(key => !userObj[key])) {
...
function* loadRepo(fullName, requiredFields) {
  ...
  //  if (!userObj || requiredFields.some(key => !userObj.hasOwnProperty(key))) {
  if (!repoObj || requiredFields.some(key => !repoObj[key])) {
...
export function* loadStarred(login, loadMore) {
 // console.log('function* loadStarred','1');
  const starredByUser = yield select(getStarredByUser, login);
 // console.log('function* loadStarred select',starredByUser);
